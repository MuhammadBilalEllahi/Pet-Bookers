...
import {ConversationItem, ChatScreenShimmer} from '../components/chat';
...
export const ChatScreen = ({navigation}) => {
  ...
  const tabs = [
    t('chatScreen.tabs.all'),
    t('chatScreen.tabs.buying'),
    t('chatScreen.tabs.selling'),
  ];
  const chips = [
    t('chatScreen.chips.all'),
    t('chatScreen.chips.unreadChats'),
    t('chatScreen.chips.important'),
  ];
    ...
    try {
      ...
      if (response.data && response.data.chat) {
        const formattedChats = response.data.chat.map(chat => {
          ...
          const isSeen = isSentByUs
            ? chat.seen_by_seller === 1
            : chat.seen_by_customer === 1;
          ...
          const isOwnAccount =
            currentSellerId && seller?.id === currentSellerId;
          ...
          if (isOwnAccount) {
            ...
            if (seller?.f_name && seller?.l_name) {
              sellerName = `${seller.f_name} ${seller.l_name} (${t(
                'common.you',
              )})`;
              subtitle = shopName;
            } else {
    ...
    try {
      ...
      if (response.data && response.data.chat) {
        const formattedChats = response.data.chat.map(chat => {
          ...
          const isOwnBuyerAccount =
            currentCustomerId &&
            (customer?.id === currentCustomerId ||
              chat.user_id === currentCustomerId);
          ...
          if (customer) {
            const baseName =
              ...
              customer.name ||
              t('common.customer');
            customerName = isOwnBuyerAccount
              ? `${baseName} (${t('common.you')})`
              : baseName;
          } else {
          ...
          return {
            ...
            title: customerName,
            imgUrl:
              customer?.image === 'def.png' || !customer?.image
                ? 'https://petbookers.com.pk/storage/app/public/profile/2024-03-26-6602afcca8664.png'
  ...
  const filteredChats = useMemo(() => {
    ...
    return chatList.filter(chat => {
      // Chip filtering
      if (
        selectedChip === t('chatScreen.chips.unreadChats') &&
        chat.unreadMessages === 0
      ) {
        return false;
      }
      if (
        selectedChip === t('chatScreen.chips.important') &&
        !chat.isImportant
      ) {
        return false;
      }
  ...
  const handleTabPress = tab => {
    if (tab === t('chatScreen.tabs.buying') && !isBuyerAuthenticated) {
      ...
      Alert.alert(t('chatScreen.authRequired.buyerTitle'), message, [
        {text: t('common.cancel'), style: 'cancel'},
        {
          text: t('chatScreen.authRequired.signInAsBuyer'),
          onPress: () =>
            navigation.navigate(AppScreens.LOGIN, {isItSeller: false}),
        },
      ]);
    ...
    if (tab === t('chatScreen.tabs.selling') && !isSellerAuthenticated) {
      ...
      Alert.alert(t('chatScreen.authRequired.sellerTitle'), message, [
        {text: t('common.cancel'), style: 'cancel'},
        {
          text: t('chatScreen.authRequired.signInAsSeller'),
          onPress: () =>
            navigation.navigate(AppScreens.LOGIN, {isItSeller: true}),
        },
      ]);
  ...
  const renderAuthPrompt = type => (
    <View style={styles.authPromptContainer}>
      ...
      <Text
        ...
        {type === 'buyer'
          ? t('chatScreen.authPrompt.buyerTitle')
          : t('chatScreen.authPrompt.sellerTitle')}
      </Text>
      <Text
        style={[
  ...
  return (
    <Layout
        ...
        {tabs.map(tab => (
          <TouchableOpacity
            ...
            <View style={styles.tabContent}>
              ...
              {((tab === t('chatScreen.tabs.buying') &&
                !isBuyerAuthenticated) ||
                (tab === t('chatScreen.tabs.selling') &&
                  !isSellerAuthenticated)) && (
                <Icon
                  name="lock-outline"
                  style={[
              ...
              {/* Show unread count */}
              {tab === t('chatScreen.tabs.buying') &&
                buyerChatList.some(chat => chat.unreadMessages > 0) && (
                  <View
                    style={[
                      styles.unreadBadge,
                    ...
                    <Text style={styles.unreadBadgeText}>
                      {
                        buyerChatList.filter(chat => chat.unreadMessages > 0)
                          .length
                      }
                    </Text>
                  </View>
              ...
              {tab === t('chatScreen.tabs.selling') &&
                sellerChatList.some(chat => chat.unreadMessages > 0) && (
                  <View
                    style={[
                      styles.unreadBadge,
                      {backgroundColor: theme['color-danger-default']},
                    ...
                    <Text style={styles.unreadBadgeText}>
                      {
                        sellerChatList.filter(chat => chat.unreadMessages > 0)
                          .length
                      }
                    </Text>
                  </View>
                )}
              {tab === t('chatScreen.tabs.all') &&
                allChats.some(chat => chat.unreadMessages > 0) && (
                  <View
                    style={[
                      styles.unreadBadge,
                      {backgroundColor: theme['color-danger-default']},
      ...
      <View style={styles.chipRow}>
        {chips.map(chip => (
          <TouchableOpacity key={chip} onPress={() => setSelectedChip(chip)}>
            <View
              style={[
