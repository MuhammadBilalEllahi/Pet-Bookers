...
import { ActivityIndicator, Alert, FlatList, Image, ScrollView, TouchableOpacity, View, Share, Modal, Dimensions, Linking } from 'react-native';
...
export const ProductDetailScreen = ({ route, navigation }) => {
    ...
    if (!isBuyerAuthenticated) {
      ...
      Alert.alert(
        ...
        [
          ...
          { text: t('product.signInAsBuyer'), onPress: () => setShowBuyerAuthModal(true) }
        ]
      );
      return;
    }
  ...
  const navigateToChat = () => {
    ...
    dispatch(
      setActiveRoom({
        ...
        recipient: {
          name: getShopName(product),
          profile: baseUrls && baseUrls['shop_image_url'] && getShopImage(product)
    ...
    navigation.navigate(ChatRoutes.MESSAGES, {
      ...
      recipientProfile: baseUrls && baseUrls['shop_image_url'] && getShopImage(product)
        ? `${baseUrls['shop_image_url']}/${getShopImage(product)}`
        : '',
  ...
  const getCategoryNames = (categoryIds) => {
    ...
    return categoryIds.map(categoryItem => {
      // Handle both formats: direct ID or object with id property
      const categoryId = typeof categoryItem === 'object' ? categoryItem.id : categoryItem;
  ...
  const handleShare = async () => {
    try {
      ...
      const response = await axiosBuyerClient.get(`products/social-share-link/${product.slug}`);
      ...
      <ScrollView
        ...
        <Layout level="1" style={{ backgroundColor: isDark ? theme['color-shadcn-background'] : theme['color-basic-100'] }}>
          <Layout
            ...
            <Layout style={{ backgroundColor: isDark ? theme['color-shadcn-background'] : theme['color-basic-100'], flex: 1 }}>
              <Layout style={[flexeStyles.row, flexeStyles.itemsCenter, spacingStyles.px4]}>
                <Text 
                  style={[
                    { 
            ...
            <Layout style={[flexeStyles.row, { backgroundColor: isDark ? theme['color-shadcn-background'] : theme['color-basic-100'] }]}>
              <Button
                accessoryLeft={
                  <ThemedIcon 
          ...
          <Layout style={{ marginHorizontal: 12, marginTop: 0, backgroundColor: isDark ? theme['color-shadcn-background'] : theme['color-basic-100'] }}>
            {product.discount > 0 ? (
              <Layout>
                <Price fontSize={20} amount={calculateDiscountedPrice(product.unit_price, product.discount, product.discount_type)} />
                <Layout style={[flexeStyles.row, flexeStyles.itemsCenter, { marginTop: 4 }]}>
                  <Text style={{ 
          ...
          <Layout style={{ flexDirection: 'row', marginTop: 18, marginBottom: 8, justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16 }}>
            ...
            >
              {product.current_stock === 0 ? t('product.outOfStock') : (addingToCart ? '' : t('product.buyNow'))}
            </Button>
            ...
            >
              {product.current_stock === 0 ? t('product.outOfStock') : (addingToCart ? '' : addedToCart ? t('product.addedToCart') : t('product.addToCart'))}
            </Button>
            ...
            <Button
              ...
              appearance="outline">{t('product.chat')}</Button>
          </Layout>
            ...
            <Layout style={[spacingStyles.p16, { backgroundColor: isDark ? theme['color-shadcn-background'] : theme['color-basic-100'] }]}
              ...
              <Layout style={{ marginTop: 16 }}>
                ...
                <Layout style={{ flexDirection: 'row', marginBottom: 8 }}>
                  <Text style={{ flex: 1, color: isDark ? theme['color-shadcn-muted-foreground'] : theme['color-basic-600'], fontSize: 14 }}>{t('product.minimumOrderQuantity')}</Text>
                ...
                <Layout style={{ flexDirection: 'row', marginBottom: 8 }}>
                  <Text style={{ flex: 1, color: isDark ? theme['color-shadcn-muted-foreground'] : theme['color-basic-600'], fontSize: 14 }}>{t('product.currentStock')}</Text>
                ...
                <Layout style={{ flexDirection: 'row', marginBottom: 8 }}>
                  <Text style={{ flex: 1, color: isDark ? theme['color-shadcn-muted-foreground'] : theme['color-basic-600'], fontSize: 14 }}>{t('product.featured')}</Text>
                  <Text style={{ flex: 1, fontWeight: '600', color: isDark ? theme['color-shadcn-foreground'] : theme['color-basic-900'], fontSize: 14 }}>{product.featured ? t('product.yes') : t('product.no')}</Text>
                </Layout>

                <Layout style={{ flexDirection: 'row', marginBottom: 8 }}>
                  <Text style={{ flex: 1, color: isDark ? theme['color-shadcn-muted-foreground'] : theme['color-basic-600'], fontSize: 14 }}>{t('product.productType')}</Text>
                ...
                <Layout style={{ flexDirection: 'row', marginBottom: 8 }}>
                  <Text style={{ flex: 1, color: isDark ? theme['color-shadcn-muted-foreground'] : theme['color-basic-600'], fontSize: 14 }}>{t('product.refundable')}</Text>
                  <Text style={{ flex: 1, fontWeight: '600', color: isDark ? theme['color-shadcn-foreground'] : theme['color-basic-900'], fontSize: 14 }}>{product.refundable ? t('product.yes') : t('product.no')}</Text>
                </Layout>

                <Layout style={{ flexDirection: 'row', marginBottom: 8 }}>
                  <Text style={{ flex: 1, color: isDark ? theme['color-shadcn-muted-foreground'] : theme['color-basic-600'], fontSize: 14 }}>{t('product.freeShipping')}</Text>
                  <Text style={{ flex: 1, fontWeight: '600', color: isDark ? theme['color-shadcn-foreground'] : theme['color-basic-900'], fontSize: 14 }}>{product.free_shipping ? t('product.yes') : t('product.no')}</Text>
                </Layout>
                ...
                {product.category_ids && product.category_ids.length > 0 && (
                  <Layout style={{ marginTop: 16 }}>
                    ...
                    <Layout style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                      {getCategoryNames(product.category_ids).map((categoryName, index) => (
                        <Text
                          key={index}
                          style={{
                ...
                {product.youtube_video_url && getYouTubeVideoId(product.youtube_video_url) && (
                  <Layout style={{ marginTop: 16 }}>
                    <Text
                      category="p1"
                      style={{
                    ...
                    <TouchableOpacity
                      onPress={() => Linking.openURL(product.youtube_video_url)}
                      style={{
                        backgroundColor: isDark ? theme['color-shadcn-card'] : theme['color-basic-100'],
                        borderRadius: 12,
                        overflow: 'hidden',
                    ...
                    >
                      <Image
                        source={{
                          uri: `https://img.youtube.com/vi/${getYouTubeVideoId(product.youtube_video_url)}/maxresdefault.jpg`
                        }}
                        style={{
                          width: '100%',
                          height: 200,
                ...
                <Layout style={{ marginTop: 16 }}>
                  ...
                  <Layout style={{ flexDirection: 'row', marginBottom: 8 }}>
                    <Text style={{ flex: 1, color: isDark ? theme['color-shadcn-muted-foreground'] : theme['color-basic-600'], fontSize: 14 }}>{t('product.address')}</Text>
                    <Text style={{ flex: 1, fontWeight: '600', color: isDark ? theme['color-shadcn-foreground'] : theme['color-basic-900'], fontSize: 14 }}>{getSellerAddress(product)}</Text>
                  </Layout>
                  <Layout style={{ flexDirection: 'row', marginBottom: 8 }}>
                    <Text style={{ flex: 1, color: isDark ? theme['color-shadcn-muted-foreground'] : theme['color-basic-600'], fontSize: 14 }}>{t('product.city')}</Text>
                    <Text style={{ flex: 1, fontWeight: '600', color: isDark ? theme['color-shadcn-foreground'] : theme['color-basic-900'], fontSize: 14 }}>{getSellerCity(product)}</Text>
                  </Layout>
                  <Layout style={{ flexDirection: 'row', marginBottom: 8 }}>
                    <Text style={{ flex: 1, color: isDark ? theme['color-shadcn-muted-foreground'] : theme['color-basic-600'], fontSize: 14 }}>{t('product.state')}</Text>
                    <Text style={{ flex: 1, fontWeight: '600', color: isDark ? theme['color-shadcn-foreground'] : theme['color-basic-900'], fontSize: 14 }}>{getSellerState(product)}</Text>
                  </Layout>
                </Layout>
              </Layout>
            </Layout>
        ...
        <View style={{ 
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
            <Image
              source={{ uri: baseUrls && baseUrls['shop_image_url'] && getShopImage(product)
                ? `${baseUrls['shop_image_url']}/${getShopImage(product)}`
                : undefined }}
              style={{ 
                width: 48, 
          ...
          <TouchableOpacity style={{ marginTop: 2, borderRadius: 8, overflow: 'hidden' }}>
            <LinearGradient
              ...
              <Text 
                onPress={() => { navigateToVandorDetail(getSellerId(product)) }} 
