...
export const fetchSellerProfileData = () => async (dispatch, getState) => {
  ...
  try {
    ...
    const profileData = {
      name: sellerData
        ? `${sellerData.f_name || ''} ${sellerData.l_name || ''}`.trim()
        : '',
      profileImage: sellerData?.image
        ? `${BASE_URLS.seller_image_url}/${sellerData.image}`
        : 'https://petbookers.com.pk/storage/app/public/profile/2024-03-26-6602afcca8664.png',
      storeName: shopData?.name || '',
      storeImage: shopData?.image
        ? `${BASE_URLS.shop_image_url}/${shopData.image}`
        : '',
    ...
    const errorMessage =
      error?.response?.data?.message ||
      error?.message ||
      'Failed to fetch seller profile data';
...
export const selectIsBuyerAuthenticated = createSelector(
  selectUserData,
  userData => {
...
export const selectIsSellerAuthenticated = createSelector(
  selectUserData,
  userData => {
...
export const selectIsAnyAuthenticated = createSelector(
  selectUserData,
  userData => {
    return (
      userData.buyerAuth.isAuthenticated || userData.sellerAuth.isAuthenticated
    );
  },
);
...
export const selectSellerProfileData = createSelector(
  selectUserData,
  userData => {
...
export const selectSellerProfileLoading = createSelector(
  selectUserData,
  userData => {
...
export const selectSellerProfileError = createSelector(
  selectUserData,
  userData => {
